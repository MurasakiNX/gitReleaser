        ...
        if int(request.headers["x-ratelimit-remaining"]) == 0:
            ...
            if request.status_code == 404:
                ...
                if not latest["assets"]:
                    ...
                    choose = input("> Do you really want to download the latest update of this GitHub repo? [Y/n] ")
                    ...
                    if choose.upper() == "N" or choose.upper() != "Y" and choose.upper() != "":
                        ...
                        customPath = input("> Do you want to change the destination folder? [Y/n] ")
                        if customPath.upper() == "N" or customPath.upper() != "Y" and customPath.upper() != "":
                            path = getcwd()
                        else:
                            path = getPath()
                        if path == ():
                        ...
                        for asset in assets:
                            ...
                            if download.upper() == "N" or download.upper() != "Y" and download.upper() != "":
                                ...
                                with open(f"{path}/{asset['name']}", "wb") as f:
                                    f.write(file.content)
                                print(f'> {asset["name"]} has been downloaded!')
                        print(f"> All desired files have been downloaded! Path: {path}")
